	...
	function browserTypeset() {
		...
		function colorWords () {
			...
			$words.each(function(i, el) {
				$(el).click(function(e) {
					var klasse = $(e.target).attr("class").split(' ')[1];
					console.log($('.' + klasse));

					// n = n == replacements.length ? 0 : n + 1;

					if(n == replacements.length) {
						r = $(e.target).data("word");
						n = 0;
					}
					else {
						r = replacements[n];
...
$.fn.wordify = function(){
   ...
   **/
	textNodes.each(function() {
		...
		var text = $this.text();
		//console.log('in  :'+text);
		text = text.replace(/([^\s-.,;:!?()[\]{}<>"]+)/g, function(v) { return '<span title="' + v + '" data-word="' + v + '" class="word ' + v.toLowerCase() + '">' + v + '</span>'; });
